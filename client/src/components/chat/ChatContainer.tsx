...
function EmptyState({ onExampleClick }: EmptyStateProps) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
      <div className="rounded-2xl bg-gradient-to-br from-primary/20 to-accent/20 p-6 mb-6 glow">
        <MessageSquare className="h-12 w-12 text-primary/80" />
      </div>
      <h2 className="text-3xl font-display font-bold gradient-text mb-1">
        ECE 350 Assistant
      </h2>
      <p className="text-xl font-display text-muted-foreground max-w-md leading-relaxed mb-4">
...
function ExampleQuestion({ question, onClick }: ExampleQuestionProps) {
  return (
    <button
      onClick={() => onClick?.(question)}
      className="text-left p-3 rounded-xl border border-border/50 bg-card/50 hover:bg-primary/10 hover:border-primary/30 transition-all duration-200 text-sm text-muted-foreground hover:text-foreground"
    >
      {question}
    </button>
  );
